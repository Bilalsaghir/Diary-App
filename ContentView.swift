// DiaryApp_SwiftUI_Multiplatform.swift
// Single-file SwiftUI demo showing a multiplatform diary app (iPhone, iPad, Mac)
// Features: entries CRUD, search, tags, calendar quick filter, personalization, reward system (points, streaks, badges), adaptive layouts
...
// MARK: - Persistence Service (JSON file for demo)
...
struct ContentView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            Text("Select or create an entry").foregroundStyle(.secondary)
        }
    ...
    var sidebar: some View {
        List {
            Section(header: headerView) {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack {
                        ...
                        ForEach(Array(Set(vm.entries.flatMap { $0.tags })).sorted(), id: \ .self) { t in
                            TagChip(tag: t, isSelected: selectedTag == t) { selectedTag = t }
                        }
                    }
                    .padding(.vertical, 4)
                }
            ...
            Section(header: Text("Pinned")) {
                ForEach(vm.entries.filter { $0.pinned && filter($0) }) { entry in
                    EntryRow(entry: entry, action: { selection = entry })
                }
            }
            ...
            Section(header: Text("All Entries")) {
                ...
                .onDelete { idx in
                    let items = vm.entries.filter { !$0.pinned && filter($0) }
                    idx.forEach { i in
                        vm.delete(items[i])
                    }
                }
            ...
            Section {
                NavigationLink(destination: RewardsView()) { Label("Rewards", systemImage: "gift") }
                NavigationLink(destination: ProfileView()) { Label("Profile", systemImage: "person.circle") }
                NavigationLink(destination: SettingsView()) { Label("Settings", systemImage: "gearshape") }
            }
        }
        .listStyle(SidebarListStyle())
        .sheet(isPresented: $showEditor) {
            EntryEditorView(initial: nil) { new in
                vm.add(new)
                showEditor = false
            }
            .environmentObject(vm)
    ...
    func filter(_ entry: DiaryEntry) -> Bool {
        let byTag = selectedTag == nil || entry.tags.contains(selectedTag!)
        let bySearch = searchText.isEmpty || entry.title.localizedCaseInsensitiveContains(searchText) || entry.body.localizedCaseInsensitiveContains(searchText) || entry.tags.contains(where: { $0.localizedCaseInsensitiveContains(searchText) })
            ...
            HStack {
                VStack(alignment: .leading) {
                    HStack {
                        ...
                        if entry.pinned { Image(systemName: "pin.fill").foregroundColor(.yellow) }
                    }
                    Text(entry.body).lineLimit(1).font(.subheadline).foregroundStyle(.secondary)
                }
                Spacer()
...
struct EntryEditorView: View {
    ...
    var body: some View {
        NavigationView {
            Form {
                Section(header: Text("Title")) { TextField("Title", text: $entry.title) }
                Section(header: Text("Body")) { TextEditor(text: $entry.body).frame(minHeight: 200) }
                Section(header: Text("Tags (comma separated)")) { TextField("e.g. travel,work", text: Binding(get: { entry.tags.joined(separator: ",") }, set: { entry.tags = $0.split(separator: ",").map { $0.trimmingCharacters(in: .whitespacesAndNewlines) } })) }
                Section(header: Text("Mood")) { TextField("Emoji (optional)", text: Binding(get: { entry.mood ?? "" }, set: { entry.mood = $0.isEmpty ? nil : $0 })) }
                Section { Toggle("Pin this entry", isOn: $entry.pinned) }
            }
            .navigationTitle(entry.title.isEmpty ? "New Entry" : "Edit Entry")
            .toolbar {
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        // if existing entry update; else create
...
struct EntryDetailView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                VStack(alignment: .leading) {
                    ...
                    HStack { Text(entry.date, style: .date); if let mood = entry.mood { Text(mood) } }
                }
                Spacer()
                Button(action: { vm.togglePin(entry); entry.pinned.toggle() }) { Image(systemName: entry.pinned ? "pin.fill" : "pin") }
            }
            ScrollView { Text(entry.body).padding(.top, 8) }
            Spacer()
        }
        .padding()
        ...
        .sheet(isPresented: $editMode) {
            EntryEditorView(initial: entry) { updated in
                entry = updated
                vm.update(updated)
                editMode = false
            }
...
struct RewardsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 16) {
                HStack { Text("Points").font(.title2); Spacer(); Text("\(vm.points)").font(.title).bold() }
                HStack { Text("Streak").font(.title2); Spacer(); Text("\(vm.currentStreak) days").font(.title).bold() }

                Divider()

                VStack(alignment: .leading, spacing: 8) {
                    Text("Badges").font(.headline)
                    if vm.badges.isEmpty { Text("Keep writing to earn badges!").foregroundStyle(.secondary) }
                    ForEach(vm.badges, id: \ .self) { badge in
                        HStack { Image(systemName: "star.fill"); Text(badge); Spacer() }
                            .padding()
                ...
                VStack(alignment: .leading) {
                    ...
                    MissionRow(title: "Write an entry today", reward: 10, completed: hasWrittenToday()) { performMission() }
                    MissionRow(title: "Tag 3 entries", reward: 20, completed: hasTagged3()) { performTagMission() }
                }
    ...
    func performMission() {
        // reward granted when mission button tapped (demo-only)
        if !hasWrittenToday() {
            vm.points += 10
    ...
    func hasTagged3() -> Bool { vm.entries.filter { $0.tags.count >= 1 }.count >= 3 }
    func performTagMission() {
        if !hasTagged3() { vm.points += 20 }
    }
}
...
struct MissionRow: View {
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading) { Text(title); Text("Reward: \(reward) pts").font(.caption).foregroundStyle(.secondary) }
            Spacer()
            if completed { Label("Done", systemImage: "checkmark.circle") }
            Button(action: action) { Text(completed ? "Claimed" : "Claim") }.disabled(completed)
        }
        .padding(8)
...
struct SettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section { Button("Export entries") { export() } }
        }
        .navigationTitle("Settings")
    }

    func export() {
        // simple export: write to shared container; for demo we'll just print
        let data = try? JSONEncoder().encode(vm.entries)
        if let d = data, let str = String(data: d, encoding: .utf8) {
            print("Exported entries:\n", str.prefix(1000))
        }
    }
}
